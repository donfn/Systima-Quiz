...
//Αυτό το function βρίσκει το στάτους του συστήματος και το μεταδίδουν στους πελάτες για να δίξουν εκείνει με την σειρά τους ένα  μεγάλο Χ εάν είναι απενεργοποιημένο
...
var admin = io.of('/admin/'); // Εδώ παρομοίως handlάρουμe την σύνδεση του Πίτερη ,,,,ΕΕΕΕΕ ΕΝΟΩ ΤΟΥ ΚΥΡΙΟΥ ΠΙΤΕΡΗ ;-;
admin.on('connection', function(socket){
	console.log('  › O διαχειριστής έχει ID:' + socket.id);
	GLOBAL.admin_socket = socket; // Και αύτο νομίζω κάπου το χρησιμοποιώ :0
    emit_status();
	socket.on('response', function(socketid,team,response){ //Όταν απαντήσει ο κύριος πίτερης στο request του μαθητή...
	console.log('  › response:' + socketid + 'data:' + response);
	    ...
	    con.query("SELECT `level` FROM teams WHERE name='"+team+"'", function (err, result, fields) {
			...
			if(result[0].level !== GLOBAL.maxlevel){//Εάν ο χρήστης δεν είναι στο τελευταίο level στέλνει τον επόμενο γρίφο
			 var currentlevel = result[0].level + 1;
			 con.query("UPDATE teams SET `level` = "+currentlevel+" WHERE `name` = '"+team+"'");
			 	    con.query("SELECT `text` FROM levels WHERE number='"+currentlevel+"'", function (err, result, fields) {
						GLOBAL.nextone = result[0].text;
						    clients.to(socketid).emit('ask_response',response); 
                            clients.to(socketid).emit('nextone',GLOBAL.nextone);
						    GLOBAL.nextone = "";// Για να είμαστε σίγουρει πως δεν θα γίνει καμία μ@λακία :)
					});
		 }
       });
		}
